<template>
  <div class="add-address">
    <div class="title">
      <el-button type="success" style="margin-top: 10px" v-if="allIntentionData != ''"
        @click="lookIntention">我的融资意向</el-button>
    </div>
    <div v-if="look === true">
      <div v-for="(item, index) in allIntentionData" :key="index" class="default-address-container">
        ...
        <div class="right-btn">
          <div class="marginR20" v-if="allIntentionData != ''" @click="handleEdit(item)">
            编辑
          </div>
          <div class="marginR20" v-if="allIntentionData != ''" @click="handleDel(item)">
            删除
          </div>
        </div>
      </div>
    ...
    <el-button type="success" style="margin-top: 10px" v-if="allIntentionData == ''"
      @click="handleAdd">添加融资意向</el-button>
    ...
    <div class="goods-box">
      <div class="goods" v-for="(item, index) in allRecommendData" :key="index" @click="detailsClick(item.userName)"
        :style="'margin-right:10px;'">
        <img class="goods-img" v-if="item.avatar != ''" :src="$store.state.imgShowRoad + '/file/' + item.avatar"
          alt="" />
        <img class="goods-img" v-else src="../assets/img/wutu.gif" style="border: 1px solid #f2f2f2" />

        <div class="info">
          <span class="initiator1" @click="detailsClick(item.userName)">姓名：{{ item.realName }}</span><br />
          <span class="initiator2" @click="detailsClick(item.userName)">联系方式：{{ item.phone }}</span><br />
          <span class="initiator3" @click="detailsClick(item.userName)">地址：{{ item.address }}</span><br />
          <span class="initiator4" @click="detailsClick(item.userName)">农作物：{{ item.item }}</span><br />
          <span class="initiator4" @click="detailsClick(item.userName)" v-if="item.amount != -1">金额：{{ item.amount
            }}元</span>
          ...
          <span class="initiator4" @click="detailsClick(item.userName)" v-if="item.area != -1">种植面积：{{ item.area }}
            亩</span>

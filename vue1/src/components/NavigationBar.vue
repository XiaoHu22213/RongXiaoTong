<template>
  <div class="navigation-bar">
    ...
    <div class="menu-content" v-cloak>
      <el-menu :default-active="$store.state.activeIndex" mode="horizontal" @select="handleSelect"
        background-color="#668d2f" text-color="#fff" active-text-color="#fff" id="menu">
        ...
        <el-submenu index="8" v-else>
          ...
          <el-menu-item index="8-2" @click="userManage" v-if="$store.getters.isAdmin">用户管理</el-menu-item>
          <el-menu-item index="8-3" @click.native="goodsManage" v-if="$store.getters.isAdmin">商品管理</el-menu-item>
          <el-menu-item index="8-4" @click.native="financeManage" v-if="isBankUser">融资管理系统</el-menu-item>
...
import { checkIsBankUser } from "../api/user"
...
export default {
  ...
  methods: {
    IsBankUser() {
      return checkIsBankUser().then(res => {
        //alert(res.data)
        this.isBankUser = res.data > 0;
      })
    },
    Login() {
    ...
    financeManage() {
      this.$router.push("/index/intelligentMatch").catch((err) => err);
    },
    financingBtn() {

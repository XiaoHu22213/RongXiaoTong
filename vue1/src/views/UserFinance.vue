<template>
  <div>
    <div v-for="(item, index) in FinanceAllData" :key="index">
      <el-descriptions class="margin-top" :column="2" :size="size" title="我的融资信息" border style="width: 100%">
        <template slot="extra">
          <!-- <span style="left"> 我的融资信息 {{index+1}}</span> -->
          <el-button type="warning" size="small" @click="updateFinaceShow(item.financeId)">修改</el-button>
          <el-button type="danger" size="small" @click="deleteFinace(item.financeId)">删除</el-button>
        </template>
    ...
    <el-dialog :title="title" :visible.sync="showUpdate" width="580px" :before-close="closeUpdate">
      <el-form v-model="FinanceData" ref="ruleForm" label-width="100px" class="demo-ruleForm">
  ...
  methods: {
    ...
    getData() {
      selectByName({}).then((res) => {
        if (res.flag == true) {
          console.log("res.data是什么" + res.data[0].bankId);
